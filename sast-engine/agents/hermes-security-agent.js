/**
 * HermesSecurityAgent — Hermes Agent Security Checks
 * ======================================================
 *
...
function checkCronSkillInjection(content, filePath, agent) {
  ...
  while ((m = SCHEDULE_RE.exec(content)) !== null) {
    ...
    findings.push(createFinding({
      ...
      cwe: 'CWE-94',
      owasp: 'ASI01',
      fix: 'Run the assembled prompt through a prompt-injection scanner before invoking the agent. Pin the skill commit hash on every scheduled run.',
    }));
  }
