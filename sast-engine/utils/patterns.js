...
export const SKIP_FILENAMES = new Set([
  ...
  'go.sum',
  'flake.lock',
  // Skip this engine's own output files to avoid scanning the report
  'sast-engine-report.html',
  'sast-engine-report.pdf',
]);
...
// SECURITY_PATTERNS/TEST_FILE_PATTERNS (vendored from upstream ship-safe) were removed here —
// neither was ever imported anywhere in this app: injection-tester.js has its own independent
// inline PATTERNS array covering the same categories, and orchestrator.js's tuneConfidence()
// does its own inline test/doc/example-path regex checks rather than importing this list.
